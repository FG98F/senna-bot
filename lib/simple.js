...
/**
    ...
    */
    ...
    let { key } = await conn.sendMessage(jid, { text: loamsg }, { quoted});

    for (let i = 0; i < loadingMessages.length; i++) {
      await conn.sendMessage(jid, { text: loadingMessages[i], edit: key }, { quoted});
    }
    await conn.sendMessage(jid, { text: loamsgEdit, edit: key }, { quoted });
  } 
  },
        //--edit messege  loading download
        loadingDl: {
    async value(jid, isLimitExceeded, msgExcd, msgd, loadingMessages, quoted) {
    let { key } = await conn.sendMessage(jid, { text: 'Cargando...' }, { quoted});

    for (let i = 0; i < loadingMessages.length; i++) {
      await conn.sendMessage(jid, { text: loadingMessages[i], edit: key }, { quoted});
    }

    const finalMessage = isLimitExceeded ? msgExcd : msgd
    await conn.sendMessage(jid, { text: finalMessage, edit: key }, { quoted });
  } 
  },
        //--
        sendHydrated: {
