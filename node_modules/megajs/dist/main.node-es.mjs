...
var abortController = globalThis.AbortController || abortControllerPolyfill;
...
var DEFAULT_HTTP_AGENT = false ? null : new HttpAgent({ keepAlive: true });
var DEFAULT_HTTPS_AGENT = false ? null : new HttpsAgent({ keepAlive: true });
var API = class _API extends EventEmitter {
...
var File = class _File extends EventEmitter2 {
  constructor(opt) {
    super();
...
import secureRandom2 from "secure-random";
...
var MutableFile = class _MutableFile extends file_default {
  ...
  mkdir(opt, originalCb) {
    ...
    if (!opt.target)
      opt.target = this;
    if (!opt.key)
      opt.key = Buffer.from(secureRandom2(16));
  ...
  _upload(opt, source, type, cb) {
    const encrypter = opt.uploadCiphertext ? new PassThrough2() : megaEncrypt(opt.key);
...
// lib/storage.mjs
var Storage = class _Storage extends EventEmitter3 {
