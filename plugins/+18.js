...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  if (!global.db.data.chats[m.chat].nsfw) throw `🚫 الجروب ده مش مفعل nsfw \n\n فعّله بــ \n*${usedPrefix}enable* nsfw`
  let user = global.db.data.users[m.sender].age
  if (user < 17) throw m.reply(`❎ لازم يكون عمرك على الأقل 18 سنة`)
  m.react(rwait)

  let res = await fetch(`https://fantox-apis.vercel.app/${command}`)
  if (!res.ok) throw await res.text()
  let json = await res.json()
  if (!json.url) throw '❎ في مشكلة'
  conn.sendFile(m.chat, json.url, 'img.jpg', `✅ صورة عشوائية ${command}`, m)
  m.react(dmoji)
}
