//لستخدام الامر تبعت كده => m.quoted

import sy////ntaxerror from 'syntax-error';
...
handl//er.customPrefix = /=?>|~/;
handl///er.command = /(?:)/i;
export/// default handler;
class CustomArray extends Array {
  construc///tor(...args) {
    if (typeof args[0] == 'number') return super(Math.min(args[0], 10000));
    else retu////rn super(...args);
  }
}
func)/tion/// pickRandom(list) {////
  return list[Math.f///loor(Math.random() * list.length)];
}//